...
(defun org-confluence-example-block (example-block contents info)
  ...
  (let ((content (org-export-format-code-default example-block info)))
    (org-confluence--block "none" "Midnight" content)))
...
(defun org-confluence-fixed-width (fixed-width contents info)
    (format "\{\{%s\}\}" (replace-regexp-in-string "\\]" "\\\\]" (replace-regexp-in-string "\\[" "\\\\[" (replace-regexp-in-string "}" "\\\\}" (replace-regexp-in-string "{" "\\\\{" contents))))))

(defun org-confluence-verbatim (verbatim contents info)
    (format "\{\{%s\}\}" (replace-regexp-in-string "\\]" "\\\\]" (replace-regexp-in-string "\\[" "\\\\[" (replace-regexp-in-string "}" "\\\\}" (replace-regexp-in-string "{" "\\\\{" (org-element-property :value verbatim)))))))

(defun org-confluence-code (code contents info)
    (format "\{\{%s\}\}" (replace-regexp-in-string "\\]" "\\\\]" (replace-regexp-in-string "\\[" "\\\\[" (replace-regexp-in-string "}" "\\\\}" (replace-regexp-in-string "{" "\\\\{" (org-element-property :value code)))))))
...
(defun org-confluence-src-block (src-block contents info)
  ...
  (let* ((lang (org-element-property :language src-block))
    ...
    (org-confluence--block language "Midnight" content)))
